...
/* Inheritance; it is possible to inherit attributes and methods from one class to another. We group the "inheritance concept" into two categories 
subclass (child) - the class that inherits from another class
superclass (parent) - the class being inherited from
To inherit from a class, use the extends keyword.
Inheritance is the concept that you can create new classes that stem from existing classes while adopting their methods and fields without actually re typing this information into the new class.
One benefit of using inheritance is that if you want to create a class that only stores slightly more information than the previous class, inheritance can save you a lot of time by offering the extends keyword. This means you no longer have to spend the time to re type all the methods and fields that you just created in your previous class.
Another benefit of using inheritance is that you can keep you program extremely organized while still keeping the same amount of information in your program.
Inheritance also allows developers to use code from other programmers' classes without copying and pasting this code into their own project. */
...
public class Main {
	...
	public static void main(String[] args) { // header and parameter
		...
		while (georgia < 34) {
			if (georgia == 32) {
				georgia++;
				continue; // The continue statement breaks one iteration (in the loop), if a specified
		...
		System.out.println(israel.getNumShekels());
		System.out.println(israel.getOriginOfCurrency());
		...
		System.out.println("\nThis program finds the smallest number in the array.\nFirst, enter how many elements you would like in the array.\nThen, enter the numbers to be stored in the array.");
		int k = scan.nextInt();
		int[] array = new int[k];
		...
		System.out.println("The follow set is the top five most used currencies' % change in buying power since 1999: " + Arrays.toString(numbers));
		   int sumOf = 0;
		   ...
		   int[][] grid = {
					{5, 32, 20433, 32, 320, 64, 19, 40, 326},
					{2, 0, 80, 20, 34, 62, 92, 49, 93},
		   ...
		   for (int row = 0; row<grid.length; row++) {
				for(int col = 0; col<grid[row].length; col++) {
					if (grid[row][col] == 19) {
						System.out.println("The value 19 has the coordinates row: "+ (row+1)+", col: "+(col+1));
					}
				}
		   }
		   ...
		   ArrayList<String> currency = new ArrayList<String>();
		    ...
		    do {
		    	try {
		    		System.out.println("Enter dollar #1 amount:");
		    		int dol1 = scan.nextInt();
		    		System.out.println("Enter dollar #2 amount:");
		    		int dol2 = scan.nextInt();
		    		int division = dol1/dol2;
